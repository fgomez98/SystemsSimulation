...
public class OffLattice {
    ...
    private List<MovingParticle> particles;
    ...
    public OffLattice(List<MovingParticle> particles, double l, int n, double appNoise) {
    ...
    public OffLattice(int n, double l, double appNoise) {
        ...
        this.L = l;
        this.M = (int) Math.ceil(l / 1); // utilizamos el M optimo
        this.particles = MovingParticle.generate(n, l, 0, 2 * Math.PI, 0.03);
        try {
    ...
    private double calculateNextAngle(MovingParticle particle, Set<Particle> neighbours) {
         /*
        ...
        */
        double cosSum = neighbours.stream().reduce(Math.cos(particle.getAngle()), (aDouble, particle1) -> aDouble + Math.cos(((MovingParticle) particle1).getAngle()), Double::sum);
        double sinSum = neighbours.stream().reduce(Math.sin(particle.getAngle()), (aDouble, particle1) -> aDouble + Math.sin(((MovingParticle) particle1).getAngle()), Double::sum);
    ...
    private void updateParticle(MovingParticle particle, Double nextAngle) {
    ...
    private void outputParticles(List<MovingParticle> movingParticles, int timeInterval, boolean append) {
        try {
            IOUtils.CSVWrite(OFF_LATICE_SIMULATION_FILENAME, movingParticles, Integer.toString(N) + "\n" + timeInterval + "\n", Particle::toString, append);
        } catch (IOException e) {
            System.err.println("An error has been encountered while writing output file");
            System.exit(1);
        }
    ...
    public static void main(String args[]) {
        ...
        List<MovingParticle> movingParticles = MovingParticle.generate(ol.N, ol.L, 0, 2 * Math.PI, 0.03);
         ...
         */
        try {
            IOUtils.CSVWriteParticles(movingParticles, OFF_LATICE_STATIC_FILENAME, OFF_LATICE_DINAMIC_FILENAME, movingParticles.size(), ol.L);
        } catch (IOException e) {
            System.err.println("An error has been encountered while writing output file");
            System.exit(1);
        }
